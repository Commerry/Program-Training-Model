...
def check_project(name):
    ...
    # Split out what the filters produced. Once generated copies are a normal
    # part of a project the total on its own stops answering the question
    # people actually have, which is whether the generating worked at all and
    # how much of the set is photographs rather than variations of them.
    entries = projects.list_images(name)
    generated = [e for e in entries if e.get('augmented')]
    originals = len(entries) - len(generated)
    if generated:
        by_preset = {}
        for entry in generated:
            marker = entry['filename'].split('_aug_')
            preset = marker[1].rsplit('_', 2)[0] if len(marker) > 1 else '?'
            by_preset[preset] = by_preset.get(preset, 0) + 1
        listed = ', '.join(f'{preset} x{count}'
                           for preset, count in sorted(by_preset.items()))
        print(f'       {originals} photographs + {len(generated)} generated '
              f'by filters')
        print(f'       {listed}')
    else:
        print(f'       {originals} photographs, none generated by filters yet')
